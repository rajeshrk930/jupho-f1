      ...
      <div className="px-4 lg:px-6 py-4 lg:py-6 space-y-6 pb-20 lg:pb-6">
        ...
        <div className="flex lg:grid overflow-x-auto lg:overflow-visible gap-3 lg:gap-4 lg:grid-cols-3 xl:grid-cols-3 -mx-4 px-4 lg:mx-0 lg:px-0 snap-x snap-mandatory scrollbar-hide">
          ...
          >
            <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-lg bg-teal-50 flex items-center justify-center mb-3 lg:mb-4">
              <Zap size={20} className="lg:w-6 lg:h-6 text-teal-600" />
            </div>
            <h3 className="font-semibold text-gray-900 mb-1 text-base lg:text-lg">New Analysis</h3>
        ...
        <div className="flex lg:grid overflow-x-auto lg:overflow-visible gap-3 lg:gap-4 lg:grid-cols-3 -mx-4 px-4 lg:mx-0 lg:px-0 scrollbar-hide">
          {isLoading || isFetching ? (
            [...Array(3)].map((_, i) => (
              <div key={i} className="stat-card border-teal-600 min-w-[160px] lg:min-w-0">
            ...
            <>
              ...
              <div className="stat-card border-teal-600 min-w-[160px] lg:min-w-0">
                ...
                <p className="text-2xl lg:text-3xl font-semibold text-gray-900 mb-1">{avgCtr ? `${avgCtr}%` : '—'}</p>
                <p className="text-xs lg:text-sm text-gray-500">Across recent</p>
              </div>
            </>
          )}
        </div>

        <section className="bg-white border border-gray-200 rounded-lg p-5 lg:p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-teal-600 font-medium uppercase tracking-wider">Recent</p>
              <h2 className="text-base lg:text-lg font-semibold text-gray-900">Recent analyses</h2>
            </div>
            <Link href="/history" className="text-sm text-teal-600 hover:text-teal-700 font-medium">View all</Link>
          </div>

          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
              {[...Array(2)].map((_, idx) => (
                <div key={idx} className="bg-white border border-gray-200 rounded-lg p-4 lg:p-5 space-y-3 shadow-sm">
                  <div className="skeleton h-3 w-24" />
            ...
            <div className="flex flex-col items-center justify-center gap-3 border-2 border-dashed border-gray-300 rounded-lg p-6 md:p-8 text-center bg-gray-50">
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-lg bg-teal-50 flex items-center justify-center">
                <BarChart3 size={24} className="md:w-7 md:h-7 text-teal-600" />
              </div>
              <p className="text-gray-900 font-semibold text-base lg:text-lg">No analyses yet</p>
              <p className="text-sm lg:text-base text-gray-600">Start your first one to see insights here.</p>
              <Link href="/analyze" className="px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-medium text-sm shadow-sm">Start first analysis</Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
              ...
              {recentSlice.map((analysis, idx) => (
                <article key={analysis.id} className={`bg-white border-l-[3px] border-teal-600 rounded-lg p-4 lg:p-5 space-y-3 shadow-sm hover:shadow-md group ${idx >= 2 ? 'hidden lg:block' : ''}`}>
                  ...
                  <p className="text-sm md:text-base font-semibold text-gray-900 line-clamp-2 group-hover:text-teal-600">{analysis.primaryReason}</p>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pt-2 border-t border-gray-100">
                    <Link href={`/history`} className="text-xs lg:text-sm text-teal-600 hover:text-teal-700 font-medium">View Details →</Link>
                    <div className="flex items-center gap-2 text-xs text-gray-700 flex-wrap font-medium">
                      <span className="px-2 py-1 bg-teal-50 text-teal-700 rounded">CTR {analysis.ctr}%</span>
                      {analysis.industry && (
        ...
        {recentSlice.length > 0 && (
          <section className="bg-white border border-gray-200 rounded-lg p-5 lg:p-6 shadow-sm space-y-4">
            ...
            >
              <div>
                <p className="text-xs text-teal-600 font-medium uppercase tracking-wider">Details</p>
                <h2 className="text-base lg:text-lg font-semibold text-gray-900">Latest analysis</h2>
              </div>
              <ChevronDown
                size={20}
                className={`lg:hidden text-gray-500 ${showLatestAnalysis ? 'rotate-180' : ''}`}
              />
            </button>
